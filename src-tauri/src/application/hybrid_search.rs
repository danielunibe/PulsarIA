...
impl BM25Engine {
    ...
    pub fn index_chunk(&mut self, job_id: i64, chunk_index: i64, text: &str) {
        ...
        self.doc_count += 1;
        self.avg_doc_len = (self.avg_doc_len * (self.doc_count - 1) as f64 + doc_len as f64) / self.doc_count as f64;
        ...
        for token in &query_tokens {
            ...
            for &(job_id, chunk_index, tf) in postings {
                ...
                let tf_norm = (tf as f64 * (self.k1 + 1.0))
                    / (tf as f64 + self.k1 * (1.0 - self.b + self.b));
                *scores.entry((job_id, chunk_index)).or_insert(0.0) += idf * tf_norm;
            }
        }

        // Convertir a SearchResult y ordenar
        let mut hits: Vec<SearchResult> = scores.into_iter().map(|((job_id, chunk_index), score)| {
            SearchResult {
                job_id,
                chunk_index,
                title: None,
...
pub fn reciprocal_rank_fusion(
    ...
    let mut all_results: Vec<SearchResult> = rrf_scores.into_iter().map(|((job_id, chunk_index), score)| {
        SearchResult {
            job_id,
            chunk_index,
            title: None,
    ...
    info!("Hybrid RRF fusion: {} BM25 + {} vector → {} finales", 
          bm25_hits.len(), vector_hits.len(), all_results.len());
    all_results
}
