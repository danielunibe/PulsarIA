...
| Datos escribibles | `db::data_dir_path()` usa `PULSAR_DATA_DIR` cuando está definido, conserva `data/` en desarrollo y usa `%APPDATA%\\Pulsaria` en instalación. El procesamiento se fija en el directorio de descargas configurable. | El paquete no intenta escribir SQLite o procesamiento dentro de Program Files. |
| Tauri | `tauri.conf.json` incluye recursos, objetivos NSIS/MSI, modo `currentUser` y el identificador `com.pulsaria.app`. | `tauri build --bundles nsis` completa y no emite el warning anterior del identificador terminado en `.app`. |
...
El instalador final se ejecutó con `/S` en `target-tauri/packaged-smoke`, una carpeta temporal aislada. Se verificó la presencia de `pulsaria.exe`, `resources/python/python.exe`, `resources/python-workers/main.py`, `resources/bin/ffmpeg.exe`, `model.onnx` y `tokenizer.json`.
...
El usuario debe ejecutar el instalador NSIS y abrir Pulsaria desde el acceso creado. La biblioteca y configuración viven en `%APPDATA%\\Pulsaria`; los videos se procesan en la carpeta de Descargas por defecto y el destino puede cambiarse desde Settings. Para una fuente TikTok, el usuario pega un enlace de video o colección autorizada; las cookies del navegador siguen siendo opt-in y no se copian ni se persisten como credenciales.
...
**Pulsaria MVP está entregado como instalador Windows NSIS funcional para las rutas locales y de runtime empaquetado.** El archivo anterior es el artefacto que debe conservarse o distribuirse. No se realizaron commits, pushes ni configuración de remotos.
